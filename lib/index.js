...
export default function init( moduleOptions ) {
  ...
  const pluginName = isProduction && !options.disabled
    ? 'plugin.js'
    : 'pluginMock.js';
  ...
  if (isProduction && options.noJS && !options.disabled && options.id !==
    emptyID) {
    ...
    this.options.head.noscript.push({
      innerHTML: `<div><img style="position:absolute;left:-9999px;" alt="ymPixel" src="https://mc.yandex.ru/watch/${options.id}"/></div>`,
      body: true,
    });
  }

  this.addPlugin(
    { src: path.resolve(__dirname, pluginName), mode: 'client', options });
}
